...
async def main(context):
    context.log("فانکشن تحلیل ترند (با عکس از وب – بدون خطا) شروع شد...")
    ...
    try:
        ...
        reportage_text = f"نمایشگر {trend_topic}: بازی Genshin Impact با ویژگی‌های جدید مثل quests حماسی، گرافیک بهتر و ایونت‌های ویژه، گیمرها رو به هیجان می‌آره! آیا آماده‌ای برای ماجراجویی؟ #GenshinImpact"
        context.log("متن engaging ساخته شد.")

        # === ۵. دانلود عکس مرتبط از وب (رایگان، کم‌حجم) ===
        possible_images = [
            ...
            "https://alphacoders.com/Genshin-Impact-wallpaper-1366x768.jpg?w=512&h=512&fit=crop"
        ]
        ...
        for img_url in possible_images:
            try:
                ...
                if img_response.status_code == 200 and len(img_response.content) < 200000:  # <200KB
                    img_data = img_response.content
                    context.log(f"عکس از وب دانلود شد (حجمه: {len(img_data)} bytes)")
                    break
        ...
        if img_data:
            # آپلود به Appwrite Storage (بدون filename)
