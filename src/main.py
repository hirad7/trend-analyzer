...
async def main(context):
    ...
    try:
        ...
        if image_response.status_code == 200:
            ...
            context.log(f"عکس در Storage آپلود شد: {final_image_url}")
        else:
            context.log(f"خطای Grok Image: {image_response.text}")

        # === ۶. ساخت data (فقط اگر image_url معتبر باشه) ===
        ...
        # فقط اگر لینک عکس معتبر باشه، اضافه کن
        if final_image_url:
