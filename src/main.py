# --- شروع اسکریپت Trend-Analyzer (نسخه نهایی ۴ - خواندن متغیرهای داخلی) ---
...
async def main(context):
    context.log("فانکشن Trend-Analyzer (نسخه نهایی ۴) شروع به کار کرد...")

    try:
        # ۱. دریافت کلیدهای مخفی
        # --- اصلاحیه: ما فقط متغیرهایی را می‌خوانیم که باید دستی تنظیم شوند ---
        GOOGLE_AI_API_KEY = os.environ.get("GOOGLE_AI_API_KEY") # از تنظیمات سراسری پروژه
        APPWRITE_API_KEY = os.environ.get("APPWRITE_API_KEY") # از تنظیمات این فانکشن
        APPWRITE_DATABASE_ID = os.environ.get("APPWRITE_DATABASE_ID") # از تنظیمات این فانکشن
        APPWRITE_COLLECTION_ID = os.environ.get("APPWRITE_COLLECTION_ID") # از تنظیمات این فانکشن
        
        # --- متغیرهای داخلی که خود Appwrite می‌دهد ---
        APPWRITE_ENDPOINT = os.environ.get("APPWRITE_ENDPOINT")
        APPWRITE_PROJECT_ID = os.environ.get("APPWRITE_PROJECT_ID")
        
        # بررسی کلیدهای ضروری
        if not (GOOGLE_AI_API_KEY and APPWRITE_API_KEY and APPWRITE_DATABASE_ID and APPWRITE_COLLECTION_ID and APPWRITE_ENDPOINT and APPWRITE_PROJECT_ID):
            context.log("خطای حیاتی: یک یا چند متغیر محیطی (API Keys یا IDها) تنظیم نشده‌اند.")
            # لاگ کردن وضعیت هر متغیر برای عیب‌یابی آسان‌تر
            context.log(f"GOOGLE_AI_API_KEY: {'OK' if GOOGLE_AI_API_KEY else 'Missing'}")
            context.log(f"APPWRITE_API_KEY: {'OK' if APPWRITE_API_KEY else 'Missing'}")
            context.log(f"APPWRITE_DATABASE_ID: {'OK' if APPWRITE_DATABASE_ID else 'Missing'}")
            context.log(f"APPWRITE_COLLECTION_ID: {'OK' if APPWRITE_COLLECTION_ID else 'Missing'}")
            context.log(f"APPWRITE_ENDPOINT: {'OK' if APPWRITE_ENDPOINT else 'Missing'}")
            context.log(f"APPWRITE_PROJECT_ID: {'OK' if APPWRITE_PROJECT_ID else 'Missing'}")
