...
async def main(context):
    context.log("فانکشن تحلیل ترند (با bucket موجود) شروع شد...")
    ...
    try:
        ...
        APPWRITE_BUCKET_ID = os.environ.get("APPWRITE_BUCKET_ID", "images")  # ID bucket موجود
        ...
        # === ۳. استفاده از bucket موجود (بدون ساخت) ===
        try:
            storage.get_bucket(APPWRITE_BUCKET_ID)
            context.log(f"Bucket '{APPWRITE_BUCKET_ID}' آماده استفاده است.")
        except Exception as e:
            context.log(f"خطای bucket: {str(e)} – مطمئن شو bucket 'images' وجود داره!")
            return context.res.json({"success": False, "error": "Bucket پیدا نشد – در Storage بساز"})
